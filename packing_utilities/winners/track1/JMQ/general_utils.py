...
def generate_multip_patten(bin_length, bin_width, box_lx, box_ly, box_num):

    n_width = bin_width // box_ly
    patten_list = []
    patten_num_list = []
    for ny in range(1,int(n_width)+1):
        for nx in range(1, int((bin_width - ny*box_ly)//box_lx) + 1):
            patten = [0] * ny
            patten.extend([1]*nx)
            ayy = []
            axx = []
            diff_gap = []
            for ay in range(1, int(box_num//ny)+1):
                if (box_num-ay*ny) % nx == 0:
                    ax = (box_num-ay*ny)//nx
                    ayy.append(ay)
                    axx.append(ax)
                    diff_gap.append(abs(ay*box_lx - ax*box_ly))
            
            if diff_gap and min(diff_gap) < min(box_lx, box_ly)*0.5:
                idx = diff_gap.index(min(diff_gap))
                patten_num = [ayy[idx]] * ny
                patten_num.extend([axx[idx]]*nx)
                patten_list.append(patten)
                patten_num_list.append(patten_num)

    return (patten_list, patten_num_list)
