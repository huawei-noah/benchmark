...
class MixedBlock:
    def __init__(self,
        ...
        """
        docstring here
            :param pt: list表示模式0-current mode, 1=rotate mode
            :param nm: 每种模式箱子的数量
            :param nz: z方向箱子个数
            :param item_size: 组成块的物品的类型
            :param weight=0: 块的总重量
        ...
        lxlist = []
        lytmp = 0
        # assert(box_lx >= box_ly)
        for i in range(len(pt)):
            if pt[i] == 0:
                lxlist.append(box_lx*nm[i])
                lytmp += box_ly
            else:
                lxlist.append(box_ly*nm[i])
                lytmp += box_lx
        self.lx = max(lxlist)
        self.minlx = min(lxlist)
        self.ly = lytmp
        self.lz = box_lz * nz
        self.vol = item_size[0] * item_size[1] * item_size[2] * sum(nm) * nz
        self.platform = platform
