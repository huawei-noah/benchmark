...
def filter_space_by_block(packed_bin, spacelist, block, space):
    shelter = False
    for space_o in spacelist:
        if space == space_o:
            continue
        if (space.min_coord[0] > space_o.min_coord[0]
                and space.min_coord[1] < space_o.max_coord[1]
                and space.min_coord[1] + block.ly > space_o.min_coord[1]
                and space.min_coord[2] < space_o.max_coord[2]
                and space.min_coord[2] + block.lz > space_o.min_coord[2]
                and space_o.lx * space_o.ly * space_o.lz > packed_bin.length * packed_bin.width*packed_bin.height*0.1):
                shelter = True
                break
...
class SSBH:
    ...
    def pack(cls,
        ...
        block_list.sort(
            key=lambda x: (x.lz * x.ly,  x.lz * x.ly * x.lx, x.lz, x.ly, x.lx), reverse=True)
        ...
        while num_residual > 0:
            ...
            while len(space_list) > 0 and num_residual > 0:
                ...
                space_list1 = copy.deepcopy(temp_space)
                space_list1.sort(key=lambda x: (x.min_coord[0]), reverse=True)
                min_x_coord = space.min_coord[0]
                x_len = space.lx
                for space1 in space_list1:
                    if space1.min_coord[0] >= min_x_coord:
                        continue
                    else:
                        if (space1.max_coord[0] == space.min_coord[0]
                                  and space1.max_coord[1] == space.max_coord[1]
                                  and space1.min_coord[1] <= space.min_coord[1]
                                  and space1.min_coord[2] == space.min_coord[2]  
                                  and space1.max_coord[2] == space.max_coord[2]):
                            min_x_coord = space1.min_coord[0]
                            coord_xspace = copy.copy(space1.min_coord)
                            coord_xspace[1] = space.min_coord[1]
                            x_len += space1.lx 
                            container_y = Space(
                                              x_len,
                                              space.ly,
                            ...
                            space_list1.remove(space1)
                if min_x_coord < space.min_coord[0]:
                    space = container_y
                    temp_space = [i for i in temp_space if i in space_list1]
                block = bh.gen_avail_block(block_list, packed_bin, space, avail)
                if block:
