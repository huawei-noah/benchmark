...
"""
""" 与packing_algorithms2.py相比较，该源代码没有后面对地面空间的组合转化过程 """
...
def update_box_size_and_avail_map(box_size_map, box_list, avail, block):
    ...
    sizes = []
    # for i in range(6):  # 原来语句
    for i in range(2):  # han add
...
def filter_space(packed_bin: PackedBin):  # (Han:判断空间有没有被box遮蔽)
    ...
    for space in space_list:
        shelter = False
        # begin: han add
        # overlap_box_list = [box for box in box_list \
        #                 if utils.is_overlap(space.min_coord, space.max_coord, \
        #                               [box.x, box.y, box.z],
...
class SSBH:
    ...
    def pack(cls,
        ...
        block_list.sort(
            key=lambda x: (x.lx * x.ly, x.lz, x.ly, x.lx), reverse=True)  # han add
        num_residual = 0
        for box in box_list:
        ...
        while num_residual > 0:
            ...
            # packed_bin.space_obj.space_list.sort(key=lambda s: (s.min_coord[0], s.min_coord[1], s.min_coord[2]), reverse=False)  # han add
            space_list.sort(key=lambda s: (s.min_coord[0], s.min_coord[1], s.min_coord[2]), reverse=False)  # han add
            # 若该容器还有可以使用的装载空间且箱子没有装载完继续装载
            # while len(packed_bin.space_obj.space_list) > 0 and num_residual > 0:
            while len(space_list) > 0 and num_residual > 0:
                # space = packed_bin.space_obj.space_list.pop(0)
                space = space_list.pop(0)
                block = gen_avail_block(block_list, bin_obj, space, avail)
                if block:
                    # if block.packed_box_list[0].box_id == "94267060100V796":
                    #     print("here")
                    ...
                    #     CoordinateItem(block, space.min_coord), space)  # 原始语句
                    space_list = space_obj.get_new_space(
                        CoordinateItem(block, space.min_coord), space, packed_bin.packed_box_list, packed_bin.width)  # han add
                    # packed_bin.space_obj.space_list = utils.update_epspace_by_box_2(packed_box_list=[packed_bin.packed_box_list[-1]], epspace_list=packed_bin.space_obj.space_list)
                    space_obj.update_epspace_by_box_2(packed_box_list=[packed_bin.packed_box_list[-1]], epspace_list=space_list)
                    # filter_space(packed_bin)
                    # packed_bin.space_obj.space_list.sort(key=lambda s: (s.min_coord[0], s.min_coord[1], s.min_coord[2]), reverse=False)  # han add
                    space_list.sort(key=lambda s: (s.min_coord[0], s.min_coord[1], s.min_coord[2]), reverse=False)  # han add
                # 若没有块能装载进当前块，进行空间转换
                # 只用当剩余空间不为空时才进行空间转换
